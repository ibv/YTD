...
// Borland's code
const
  faReadOnly  = $00000001 {$IFDEF PEPAK}{$IFDEF DELPHI6_UP} platform {$ENDIF}{$ENDIF} ;
  faHidden    = $00000002 {$IFDEF PEPAK}{$IFDEF DELPHI6_UP} platform {$ENDIF}{$ENDIF} ;
  faSysFile   = $00000004 {$IFDEF PEPAK}{$IFDEF DELPHI6_UP} platform {$ENDIF}{$ENDIF} ;
  faVolumeID  = $00000008 {$IFDEF PEPAK}{$IFDEF DELPHI6_UP} platform {$ENDIF}{$ENDIF} ;
  faDirectory = $00000010;
  faArchive   = $00000020 {$IFDEF PEPAK}{$IFDEF DELPHI6_UP} platform {$ENDIF}{$ENDIF} ;
  faSymLink   = $00000040 {$IFDEF PEPAK}{$IFDEF DELPHI6_UP} platform {$ENDIF}{$ENDIF} ;
  faAnyFile   = $0000003F;
{ File open modes }
{$ifdef Win32}
  fmOpenRead       = $0000;
...
type
  ...
  TSearchRec = record
    ...
    ExcludeAttr: integer;
{$ifdef Win32}
    FindHandle: THandle  {$IFDEF PEPAK}{$IFDEF DELPHI6_UP} platform {$ENDIF}{$ENDIF} ;
    FindData: TWin32FindData  {$IFDEF PEPAK}{$IFDEF DELPHI6_UP} platform {$ENDIF}{$ENDIF} ;
{$else}
...
procedure YearToPChar(Y: Word; P: PAnsiChar);
...
function IntToStr(Value : integer): string; // 3x faster than SysUtils.IntToStr
...
function IdentToColor(Ident: PAnsiChar): integer;
type
...
asm // faster version by AB
  ...
  mov eax,edx
  call StrLen
  lea ecx,eax+1 // also copy last #0
  pop eax
...
function FileCreate(const FileName: string): integer;
begin
  result := integer(CreateFile(pointer(FileName), GENERIC_READ or GENERIC_WRITE,
  ...
  if (Hour < HoursPerDay) and (Min < MinsPerHour) and (Sec < SecsPerMin)
   and (MSec < MSecsPerSec) then
    result := (Hour * (MinsPerHour * SecsPerMin * MSecsPerSec) +
             Min * (SecsPerMin * MSecsPerSec) + Sec * MSecsPerSec +
             MSec) / MSecsPerDay else
    result := 0;
end;
...
function DateTimeToPChar(DateTime: TDateTime; dest: pChar): pChar;
...
begin
  ...
  YearToPChar(Y,result);
  result[4] := '/';
  pWord(result+5)^  := pWord(@TwoDigitLookup[M])^;
  result[7] := '/';
  pWord(result+8)^  := pWord(@TwoDigitLookup[D])^;
  result[10] := ' ';
  pWord(result+11)^  := pWord(@TwoDigitLookup[H])^;
  result[13] := ':';
  pWord(result+14)^ := pWord(@TwoDigitLookup[MI])^;
  result[16] := ':';
  pWord(result+17)^ := pWord(@TwoDigitLookup[S])^;
  inc(result,19);
end;
...
begin
  SetLength(result,19);
  DateTimeToPChar(DateTime, pointer(result));
end;
...
begin
  ...
  while (i<=L) do begin
    ...
    if (Format[i]='.') and (i+2<=L) and (c<=high(Args)) and (Format[i+1] in ['1'..'9'])
      and (Format[i+2] in ['d','x','p']) and (Args[c].VType=vtInteger) then begin
      j := Args[c].VInteger;
...
{      'g','f','n','m': case VType of
      ...
      'x','p': if VType=vtInteger then
        result := result+IntToHex(VInteger,1);
      end;
      inc(c);
    end;
    inc(i);
...
begin
  raise Exception.CreateFmt('Error %d at %x',[ErrorCode,integer(ErrorAddr)])
    at ErrorAddr;
end;
...
begin
  ...
  if Drive > 0 then begin
    inc(Drive,$40+ord(':')shl 8+ord('\')shl 16);
    RootPtr := @Drive;
  end;
