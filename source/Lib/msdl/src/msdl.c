...
#ifdef DLL
    dlopts->tag = options->tag;
    dlopts->downloadProgressCallback = options->downloadProgressCallback;
#endif
...
static int is_metafile(char *name)
{
	...
	for(i = 0; i < sizeof(metafile_prefixes)/sizeof(char *) ; i++) {
	    if(!strcasecmp(metafile_prefixes[i],p)) return 1;
	}
    }
	...
	for(i = 0; i < sizeof(metafile_starts)/sizeof(char *) ; i++) {
	    if(!strncasecmp(metafile_starts[i],p,strlen(metafile_starts[i]))) return 1;
	}
    }
...
#ifdef DLL
	    if (dlopts->downloadProgressCallback)
	    {
	      int stop_it = 0;
	      dlopts->downloadProgressCallback(dlopts->tag, size_written, stream->stream_ctrl->file_size, &stop_it);
	      if (stop_it)
	        goto failed;
	    }
#endif
