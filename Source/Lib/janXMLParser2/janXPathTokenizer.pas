...
History:
  1.1 15-nov-2003 :
    - changed division operator / to div
    - included / in identchars
    - detect / containing identifiers as XPath
...
type
  ...
  TTokenOperator=(toNone,toString,toNumber, toAttribute, toElement,
     toName, toPosition, toFirst, toLast, toPath,toValue, toParentName, toChildCount, toNameSpace,
     toBaseName, toNameSpaceURI,toLocation,
...
begin
  ...
  if bot='''' then begin  // string
    ...
    if lowercase(FToken)='value' then begin
        FTokenKind:=tkOperand;
        FTokenValue:='';
    ...
    else if lowercase(FToken)='position' then begin
        FTokenKind:=tkOperand;
        FTokenValue:='';
        FTokenOperator:=toPosition;
    end
    else if lowercase(FToken)='first' then begin
        FTokenKind:=tkOperand;
        FTokenValue:='';
        FTokenOperator:=toFirst;
    end
    else if lowercase(FToken)='last' then begin
        FTokenKind:=tkOperand;
        FTokenValue:='';
        FTokenOperator:=toLast;
    end
    else if lowercase(FToken)='path' then begin
        FTokenKind:=tkOperand;
        FTokenValue:='';
        FTokenOperator:=toPath;
    end
    else if lowercase(FToken)='div' then begin
      FTokenKind:=tkOperator;
      FTokenOperator:=toDivide;
      FTokenLevel:=5      
    end
    ...
    else if lowercase(FToken)='basename' then begin
        FTokenKind:=tkOperand;
        FTokenValue:='';
        FTokenOperator:=toBaseName;
    end
    ...
    else if lowercase(FToken)='namespace' then begin
        FTokenKind:=tkOperand;
        FTokenValue:='';
        FTokenOperator:=toNameSpace;
    end
    else if lowercase(FToken)='namespaceuri' then begin
        FTokenKind:=tkOperand;
        FTokenValue:='';
        FTokenOperator:=toNameSpaceURI;
    end
...
begin
  ...
  else if value='assign' then begin
    FtokenKind:=tkOperator;
    FTokenOperator:=toAssign;
    FtokenLevel:=7;
    result:=true;
  end
  else if value='get' then begin
    FtokenKind:=tkOperator;
    FTokenOperator:=toGet;
    FtokenLevel:=7;
    result:=true;
  end
